...
class Debugger(object):
  ...
  def gen_colormap(self, img, name, output_res=None):
    img = img.copy()
    # ignore region
    img[img == 1] = 0.5
  ...
  def add_arrow(self, st, ed, img_id, c=(255, 0, 255), w=2,j=False):
    c = (0, 0, 255)
    # if j > 2:
    #     c = (0, 0, 255)
    # else:
    #     cc = [(0, 0, 255),(125, 255, 0),(255, 0, 125)]
    #     c = cc[j]
    cv2.arrowedLine(
      self.imgs[img_id], (int(st[0]), int(st[1])), 
      (int(ed[0] + st[0]), int(ed[1] + st[1])), c, 5,
