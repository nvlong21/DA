...
class centerFace_Utils:
    ...
    def detect(self, img, output, threshold=0.5, im_scale=1.0):
        ...
        for idx, s in enumerate(self._feat_stride_fpn):
            ...
            key_anchor = '%dx%d'%(height, width)
            A = self._num_anchors['stride%s'%s]
            if key_anchor not in self._anchors_plane.keys():
                self._anchors_plane[key_anchor] = {}
                if 'stride%s'%s not in self._anchors_plane[key_anchor].keys():
                    anchors_fpn = self._anchors_fpn['stride%s'%s]
                    K = height * width
                    self._anchors_plane[key_anchor]['stride%s'%s] = anchors_plane(height, width, stride, anchors_fpn).reshape((K * A, 4))

            anchors = self._anchors_plane[key_anchor]['stride%s'%s]
            ...
            proposals[:,0::2] /= im_scale
            proposals[:,1::2] /= im_scale
            ...
            if not self.vote and self.use_landmarks:
                ...
                landmarks[:,:,0:1] /= im_scale
                landmarks[:,:,1:2] /= im_scale
